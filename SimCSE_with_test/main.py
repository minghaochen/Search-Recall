...
if __name__ == "__main__":
    ...
    log_file = prepare()
    model = TextBackbone().cuda()
    model.load_state_dict(torch.load("./output/unsup_epoch_1.pt"))
    print(model.use_drop)
    dataset = Supervised()
    dataloader = DataLoader(dataset, batch_size=64, shuffle=True, drop_last=True)
    param_optimizer = list(model.named_parameters())
    no_decay = ["bias", "LayerNorm.bias"]
    ...
    # query2doc
    query2doc = {}
    with open('query2doc.txt', 'r', encoding='utf-8') as f:
        for line in f.readlines():
            line = line.strip().split("\t")
            query2doc[line[0]] = eval(line[1])
    all_data = {}
    ...
    for idx, x in tqdm(testloader):
        with torch.no_grad():
            if idx in query2doc:
                y0 = model.predict(x)[0].detach().cpu().numpy()
                for i in range(min(len(query2doc[idx],5))):
                    text = all_data[query2doc[idx][i]]
                    data = testdata.tokenizer(text,
                                          truncation=True,
                                          add_special_tokens=True,
                                          max_length=testdata.length,
                                          padding='max_length',
                                          return_tensors='pt').to("cuda:0")
                    y0 += model.predict(data)[0].detach().cpu().numpy()
                y = y0 / (min(len(query2doc[idx],5)) + 1)
                y = y.tolist()
            else:
