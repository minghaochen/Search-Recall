...
class TextBackbone(torch.nn.Module):
    def __init__(self,
                 pretrained='cyclone/simcse-chinese-roberta-wwm-ext',
                 output_dim=128) -> None:
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        if self.use_drop:
            ...
            logits1 = self.fc(self.dropout1(pooled_output))
            logits2 = self.fc(self.dropout2(pooled_output))
            logits3 = self.fc(self.dropout3(pooled_output))
            logits4 = self.fc(self.dropout4(pooled_output))
            logits5 = self.fc(self.dropout5(pooled_output))
