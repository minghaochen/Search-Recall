...
class Supervised(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        neg_text1 = self.all_data[str(tmp)]
        tmp = random.randint(1, 1001492)
        neg_text2 = self.all_data[str(tmp)]
        tmp = random.randint(1, 1001492)
        neg_text3 = self.all_data[str(tmp)]
        tmp = random.randint(1, 1001492)
        neg_text4 = self.all_data[str(tmp)]
        tmp = random.randint(1, 1001492)
        neg_text5 = self.all_data[str(tmp)]

        sample = self._post_process(anchor_text, pos_text,
                                    neg_text1, neg_text2, neg_text3,
                                    neg_text4, neg_text5)
        return sample

    def _post_process(self, anchor_text, pos_text, neg_text1, neg_text2, neg_text3, neg_text4, neg_text5):
        sample = self.tokenizer([anchor_text, pos_text, neg_text1, neg_text2, neg_text3, neg_text4, neg_text5],
                                truncation=True,
                                add_special_tokens=True,
                                max_length=48,
                                padding='max_length',
